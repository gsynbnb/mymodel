...
class SODADSplit(object):
    ...
    def _getAnnandImg(self):
        ...
        """
        self._loadAnn()
        #defaultdicr=>{imgid:list[dict(标注信息)]}
        self.imgToAnns = self.annInfo.imgToAnns
        #dict=>{imgid:dict{图片信息}}
    ...
    def _getIgnMask(self, maskSz, catLst, bounds, left, up):
        ...
        objBboxAry = np.array(bboxLst)[~ignSign].astype(np.int_)
        ignBboxAry = np.array(bboxLst)[ignSign].astype(np.int_)
