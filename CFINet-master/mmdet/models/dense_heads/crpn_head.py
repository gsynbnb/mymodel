...
from mmdet.core import bbox2result, bbox2roi, build_assigner, build_sampler,build_bbox_coder
...
class Multigranularity_FF(nn.Module):
    def __init__(self):
        super(Multigranularity_FF,self).__init__()
        ...
        self.gap8=nn.AdaptiveAvgPool2d(1)
        self.gap9=nn.AdaptiveAvgPool2d(1)
        self.gap10=nn.AdaptiveAvgPool2d(1)
        self.lin1=nn.Linear(512,256)
        self.lin2 = nn.Linear(512, 256)
        self.lin3 = nn.Linear(512, 256)
        self.lin4 = nn.Linear(512, 256)
        self.lin5 = nn.Linear(512, 128)
        self.lin6 = nn.Linear(512, 128)
        self.lin7 = nn.Linear(512, 128)
        self.lin8 = nn.Linear(512, 128)
    def forward(self,x):
        #x is list
        x1,x2,x3,x4=x
        g1=self.gap1(x4)
        ...
        g2=self.gap2(x3)
        g2=torch.flatten(g2,start_dim=1)
        x3_1=x2[:,:,:76,:]
        x3_2=x2[:,:,76:,:]
        x3_3=x2[:,:,:,:76]
        x3_4=x2[:,:,:,76:]
        ...
        x4_3=x1[:,:,152:,:152]
        x4_4=x1[:,:,152:,152:]
        g3_u=self.gap3(x3_1).flatten(start_dim=1)
        g3_d=self.gap4(x3_2).flatten(start_dim=1)
        g3_r=self.gap5(x3_4).flatten(start_dim=1)
        g3_l=self.gap6(x3_3).flatten(start_dim=1)
        g4_lu=self.gap7(x4_1).flatten(start_dim=1)
        g4_ru=self.gap8(x4_2).flatten(start_dim=1)
        g4_ld=self.gap9(x4_3).flatten(start_dim=1)
        g4_rd=self.gap10(x4_4).flatten(start_dim=1)
        g4_l=self.lin1(torch.cat((g4_lu,g4_ld),dim=1))
        g4_u=self.lin2(torch.cat((g4_ru,g4_lu),dim=1))
        g4_r=self.lin3(torch.cat((g4_rd,g4_u),dim=1))
        g4_d=self.lin4(torch.cat((g4_ld,g4_r),dim=1))
        g_u=self.lin5(torch.cat((g3_u,g4_u),dim=1))
        g_d=self.lin6(torch.cat((g3_d,g4_d),dim=1))
        g_r=self.lin7(torch.cat((g3_r,g4_r),dim=1))
        g_l=self.lin8(torch.cat((g3_l,g4_l),dim=1))
        FusionFeature=torch.cat([g1,g2,g_l,g_u,g_r,g_d],dim=1)
        return FusionFeature

class GATNet(nn.Module):
    def __init__(self):
        super(GATNet,self).__init__()
        self.conv1=GATConv(1024,128,heads=2)
        self.lin1=nn.Linear(256,128)
        self.predict_cls=nn.Linear(128,2)
        self.predict_bbox=nn.Linear(128,4)
    ...
    def forward(self,data):
        ...
        x=self.bn1(x)
        x=self.act1(x)
        scores=self.predict_cls(x[:size*600])
        predict_bbox=self.predict_bbox(x[:size*600])
        return  scores,predict_bbox
...
class CRPNHead(BaseDenseHead, BBoxTestMixin):
    def __init__(self, num_stages,loss_cls,loss_bbox,assigner,bbox_roi_extractor,bbox_coder1,bbox_head,stages, train_cfg, test_cfg, init_cfg=None):
        ...
        self.bbox_coder=build_bbox_coder(bbox_coder1)
        self.bbox_assigner = build_assigner(assigner)
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self.clsloss=build_loss(loss_cls)
        self.bboxloss=build_loss(loss_bbox)
        self.squential=nn.Sequential(
            nn.Linear(12544,1024),
            nn.Linear(1024,1024)
        )
        self.MGFF=Multigranularity_FF()
        self.GATModule=GATNet()
    ...
    def get_refinedpropoasals(self,proposal_lists,predict_boxes,scores,img_metas):
        Scores=torch.softmax(scores,dim=-1,dtype=torch.float)[:,1].view(-1)
        img_shape = img_metas[0]['img_shape']
        refine_proposal_list=[]
        for  i,pre_proposal in enumerate(proposal_lists):
            decode_proposal=self.bbox_coder.decode(pre_proposal[:,:4],predict_boxes[600*i:600*(i+1)],max_shape=img_shape)
            Scores1=Scores[600*i:600*(i+1)]
            refine_proposal,_=soft_nms(decode_proposal,Scores1,iou_threshold=0.75,sigma=0.5,min_score=0.005)
            refine_proposal=refine_proposal[:300].detach()
            refine_proposal_list.append(refine_proposal)

        return  refine_proposal_list
    ...
    def forward_train(self,
        ...
        if proposal_cfg is None:
            ...
            indexes_s1=[]
            encode_bboxess=[]
            index=torch.arange(0,600)
            for i in range(len(img_metas)):
               assign_result = self.bbox_assigner.assign(
                proposal_list[i], gt_bboxes[i], None,None)
               s1=torch.gt(assign_result.gt_inds,0)
               s2=torch.eq(assign_result.gt_inds,0)
               index[s1]=1
               index[s2]=0
               if torch.nonzero(assign_result.gt_inds).size(0)==0:
                   gtb=torch.zeros((s1.size(0),4))
                   encode_bboxes=torch.tensor([])

               else:
                   gtb=gt_bboxes[i][assign_result.gt_inds[s1]-1]
                   ptb=proposal_list[i][s1][:,:4]
                   encode_bboxes=self.bbox_coder.encode(ptb,gtb)

               indexes_s1.append(s1)
               encode_bboxess.append(encode_bboxes)
               indexes.append(index)
               assign_results.append(assign_result)


            proposal_labels=torch.cat(indexes,dim=-1)

               # assign_results.append(assign_result)
            Feture_s=self.MGFF(featmap)
            rois = bbox2roi([res[:,:4] for res in proposal_list])
            confidence=torch.cat([res[:,4] for res in proposal_list],dim=-1)
            bbox_feats = self.bbox_roi_extractor(
                featmap[:self.bbox_roi_extractor.num_inputs], rois)
            bbox_feats=torch.flatten(bbox_feats,start_dim=1)
            bbox_feats_GC=self.squential(bbox_feats)
            bbox_feats_GC=torch.cat((bbox_feats_GC,Feture_s),dim=0)
            threshold=torch.mean(confidence,dim=-1)
            tea_labels=(confidence>=threshold).split(600,dim=-1)
            stu_labels=(confidence<threshold).split(600,dim=-1)
            # a=list(zip(tea_labels,stu_labels))
            edge_indexf=[]
            for i,tuple1 in enumerate(zip(tea_labels,stu_labels)):
                 tea_labels_i,stu_labels_i=tuple1
                 poslabels=torch.nonzero(tea_labels_i)+600*i
                 neglabels=torch.nonzero(stu_labels_i)+600*i
                 x1,x2=neglabels.size(0),poslabels.size(0)
                 e_s=poslabels.repeat(1,x1).view(-1)
                 e_t=neglabels.flatten(start_dim=0).repeat(x2)
                 edge_index=torch.stack((e_s,e_t),dim=0)
                 AXU=neglabels.flatten(start_dim=0)
                 feature_s=torch.ones_like(AXU)*(600*len(img_metas)+i)
                 edge_index_=torch.stack((feature_s,AXU),dim=0)
                 edge_index=torch.cat((edge_index,edge_index_),dim=-1)
                 edge_indexf.append(edge_index)
            edge=torch.cat(edge_indexf,dim=-1)
            data=Data(x=bbox_feats_GC,edge_index=edge)
            scores,predict_bbox=self.GATModule(data)
            focal_loss=self.clsloss(scores,proposal_labels.to(device=torch.device("cuda:0"),dtype=torch.long),None,scores.size(0))
            bbox_loss=torch.tensor(0.0,dtype=torch.float,device=torch.device("cuda:0"))

            for i,result1 in enumerate(zip(encode_bboxess,indexes_s1)):
                ecodebbox,indexes1=result1
                if ecodebbox.size(0)==0:
                    loss1=self.bboxloss(predict_bbox[600*i:600*(i+1)],ecodebbox)
                else:
                    loss1=self.bboxloss(predict_bbox[600*i:600*(i+1)][indexes1],ecodebbox,avg_factor=600)
                bbox_loss+=loss1
            bbox_loss/=len(img_metas)
            list1=[]
            list2=[]
            list1.append(focal_loss)
            list2.append(bbox_loss)
            losses['s3.loss_greason_fcolloss']=list1
            losses['s3.loss_greason_bboxloss']=list2
            proposal_list=self.get_refinedpropoasals(proposal_list,predict_bbox,scores,img_metas)
